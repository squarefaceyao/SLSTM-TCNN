...
for i in range(9):
    if DS == 'CNN':
        name = [name for name in os.listdir(savePath)
                      if fnmatch(name, f'DSC{i+1}_40.csv')]  # 获取path路径后缀为.h5的文件
        model_name= 'STCM'
    else:
        name = [name for name in os.listdir(savePath)
                      if fnmatch(name, f'DSCP{i+1}_148.csv')]  # 获取path路径后缀为.h5的文件
        model_name = 'STCM+SLSTM-TCNN'
    ...
    result[salt[i]] = kfold_cnn_2(inputs=xx_, targets=y_train,salt=i,
                                  datasets = pa[2:-2],epochs=epochs,
                                  save_model=1)

now = int(round(time.time() * 1000))
localtime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(now / 1000))
pd.DataFrame(result).to_csv(f"result/STCM_{localtime}.csv")
